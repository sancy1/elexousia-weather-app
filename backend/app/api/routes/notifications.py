...
#     """
#     Deletes all notifications (both read and unread) for the current user
#     """
#     user_id = user.id
...
#             cursor = conn.cursor()
#             cursor.execute(
#                 "DELETE FROM weather_notifications WHERE user_id = %s",
#                 (user_id,)
#             )
#             conn.commit()
...
# ⚠️ IMPORTANT: DELETE ALL MUST COME BEFORE /{notification_id}
@router.delete("/delete-all")
async def delete_all_notifications(user = Depends(get_current_user)):
    """
    Deletes all notifications (both read and unread) for the current user

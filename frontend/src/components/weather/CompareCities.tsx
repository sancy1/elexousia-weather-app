  ...
  return (
    <div className="relative group">
      ...
      <input
        ...
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full h-10 pl-9 pr-3 rounded-xl bg-black/40 text-white placeholder:text-gray-400 border border-primary/40 focus:border-primary/60 focus:ring-2 focus:ring-primary/30 focus:bg-black/60 outline-none text-[13px] font-medium transition-all"
      />
    </div>
  );
}
    ...
    <div className={`relative rounded-2xl border border-border bg-gradient-to-br ${accent} p-4 overflow-hidden group hover:border-primary/30 transition-all`}>
      ...
      <div className="relative flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <h3 className="mt-0.5 text-[18px] font-bold tracking-tight truncate text-white">{data.city}</h3>
        </div>
        {isWarmer ? (
          <span className="shrink-0 flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-500 border border-amber-500/30">
            <TrendingUp className="h-2.5 w-2.5" /> Warmer
          </span>
        ) : (
          <span className="shrink-0 flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-sky-500/15 text-sky-500 border border-sky-500/30">
            <TrendingDown className="h-2.5 w-2.5" /> Cooler
          </span>
        )}
      </div>
      ...
      <div className="relative mt-3 flex items-end gap-2">
        <span className="text-[44px] font-bold leading-none tracking-tighter text-white">{Math.round(temp)}°</span>
        <span className="mb-1.5 text-[12px] text-gray-400">Feels {Math.round(feels)}°</span>
      </div>
      <p className="relative mt-1 text-[12px] text-gray-300 line-clamp-1">{data.condition}</p>
...
function Stat({ icon: Icon, value, label }: { icon: typeof Thermometer; value: string; label: string }) {
  return (
    <div className="rounded-lg bg-black/30 border border-border/60 p-2 backdrop-blur-sm">
      <Icon className="h-3 w-3 text-gray-400" />
      <div className="mt-1 text-[12px] font-semibold leading-none text-white">{value}</div>
      <div className="mt-1 text-[9px] uppercase tracking-wide text-gray-500">{label}</div>
    </div>
  );
}